...
export class MixPanel150 extends Device {
  ...
  public runCCTLoop({
    ...
    this.logger.info(`setHUE(${String(from)}, ${String(to)}, ${String(speed)}, ${direction}) => [${String(valueFromCCT)}, ${String(valueToCCT)}]`);
  ...
  public setAction(action: unknown): ThrowsException<void> {
    ...
    if (this.is(SetHUEApi, action)) { this.setHUE(action.parameters); return; }
    if (this.is(RunCCTLoopApi, action)) { this.runCCTLoop(action.parameters); return; }
    if (this.is(SetLightModeApi, action)) { this.setLightMode(action.parameters); return; }
    if (this.is(SetBrightnessApi, action)) { this.setBrightness(action.parameters); return; }
    if (this.is(SetLightDiffuserApi, action)) { this.setLightDiffuser(action.parameters); return; }
    if (this.is(SetColorTemperatureApi, action)) { this.setColorTemperature(action.parameters); return; }
    if (this.is(SetGreenMagentaBiasApi, action)) { this.setGreenMagentaBias(action.parameters); return; }
    if (this.is(SetSaturationApi, action)) { this.setSaturation(action.parameters); return; }
  }
