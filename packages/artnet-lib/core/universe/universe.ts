...
export class Universe {
    ...
    public getPortAddressInfo(): { net: number; subNet: number; universe: number } {
        return {
    ...
    public getSize(): number {
        return Array.from(this.devices.entries()).reduce(
            (acc, [_, device]) => acc + device.getDmxDataSize(),
            0,
        );
    }
    ...
    public getDevice
    <TDeviceKey extends keyof SupportedDevices>
    (index: number)
    ...
    public getDevice
    <TDeviceKey extends keyof SupportedDevices>
    (indexOrName: number | TDeviceKey): InstanceType<valueOf<SupportedDevices>> | InstanceType<valueOf<SupportedDevices>>[] | null
    {
